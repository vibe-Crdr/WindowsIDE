...
namespace WindowsIDE.Host.Csharp
{
    /// <summary>
    /// ユーザー EXE の 1 行。stdout か stderr か。
    /// </summary>
    public sealed class CsharpLineReceivedEventArgs : EventArgs
    {
        ...
        public CsharpLineReceivedEventArgs(int generation, string text, bool isStderr)
        {
    ...
    /// ユーザー EXE の終了。
    /// </summary>
    public sealed class CsharpProcessExitedEventArgs : EventArgs
    {
        private int generation;
        ...
        public CsharpProcessExitedEventArgs(int generation, int exitCode)
        {
            this.generation = generation;
    ...
    /// ユーザー EXE の起動失敗。
    /// </summary>
    public sealed class CsharpStartFailedEventArgs : EventArgs
    {
        private int generation;
        ...
        public CsharpStartFailedEventArgs(int generation, string message)
        {
            this.generation = generation;
    ...
    /// <summary>
    /// 手動ビルド成功後のユーザー EXE を 1 本だけ起動する。csc は持たない。WinForms 非依存。
    /// 保持している Process 参照だけ Kill する。プロセス名検索はしない。
    /// </summary>
    public sealed class CsharpProcessHost
    {
        ...
        public event EventHandler<CsharpLineReceivedEventArgs> LineReceived;

        /// <summary>終了。呼び出しスレッドは UI ではない。</summary>
        public event EventHandler<CsharpProcessExitedEventArgs> Exited;

        /// <summary>起動失敗。</summary>
        public event EventHandler<CsharpStartFailedEventArgs> StartFailed;

        /// <summary>
        /// 既存プロセスを Kill してから exePath を起動する。引数は空。stdin は直後に閉じる。
        /// 呼び出しスレッドで WaitForExit しない。
        /// </summary>
        /// <param name="exePath">その回の手動ビルドが書いた EXE。</param>
        /// <param name="workingDirectory">作業ディレクトリ。空なら EXE のディレクトリ。</param>
        /// <param name="generation">古い完了を捨てるための世代。</param>
        public void Start(string exePath, string workingDirectory, int generation)
        {
            ...
            {
                work = Path.GetDirectoryName(exePath);
            }
        ...
        {
            EventHandler<CsharpLineReceivedEventArgs> h = this.LineReceived;
            if (h != null)
            {
                h(this, new CsharpLineReceivedEventArgs(generation, text, isStderr));
            }
        }

        private void RaiseExited(int generation, int exitCode)
        {
            EventHandler<CsharpProcessExitedEventArgs> h = this.Exited;
            if (h != null)
            {
                h(this, new CsharpProcessExitedEventArgs(generation, exitCode));
            }
        }

        private void RaiseStartFailed(int generation, string message)
        {
            EventHandler<CsharpStartFailedEventArgs> h = this.StartFailed;
            if (h != null)
            {
                h(this, new CsharpStartFailedEventArgs(generation, message));
            }
        }
